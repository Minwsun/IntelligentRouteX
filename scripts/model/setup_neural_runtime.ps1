...
function Install-Python311 {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        Write-Host "[runtime] Installing Python 3.11 with winget..."
...
$venvDir = Join-Path $RepoRoot ".venv-neural"
if (-not (Test-Path -LiteralPath $venvDir)) {
    Write-Host "[runtime] Creating venv: $venvDir"
...
Write-Host "[runtime] Bootstrap python: $pythonExeForVenv"
Write-Host "[runtime] Python ready: $venvPython"
& $venvPython --version
